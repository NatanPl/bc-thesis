...
class GAIN(pl.LightningModule):
    """
    Generative Adversarial Imputation Network that operates on 2D arrays `(n_indicators, n_years)`.

    One training sample = one panel.  Batching is handled by the
    `DataLoader`, so every tensor that reaches this module is either
    * `shape == (batch, n_indicators, n_years)`, or
    * `shape == (n_indicators, n_years)` for a single sample.
    """

    def __init__(
        self,
        data_shape: Tuple[int, int],       # (n_indicators, n_years)
        *,
        hint_rate: float = 0.9,
        alpha: float = 10.0,
        ...
        n_indicators, n_years = data_shape
        self.input_dim = n_indicators * n_years
        ...
        self.alpha = alpha
        self.register_buffer("adv_scale", torch.tensor(0.1))
    ...
    def _gen_step(self, flat, orig_observ):
        ...
        # if ((self.alpha * self.adv_scale * adv / recon) > 0.5 and recon > 1e-3):
        #     print("Warning: dial scale down")
...
class GAINImputer:
    ...
    enumerates samples (e.g. countries).  Internally the model itself only
    sees 2-D panels (indicators x years)."""
    ...
    def _norm(self, x: np.ndarray) -> np.ndarray:
        ...
        if self._min is None:
            # Calculate min/max carefully to avoid NaN propagation
    ...
    def _loaders(self, t: torch.Tensor):
        ...
        return (
            DataLoader(train_ds, batch_size=len(train_ds),
    ...
    def fit(self, data: np.ndarray):
        ...
        # Create model with appropriate shape parameters
        if data.ndim == 3:  # If 3D array (samples, indicators, years)
            data_shape = data.shape[1:]  # (indicators, years)
        else:
            data_shape = data.shape  # Assume already correct shape
    ...
    def transform(self, data: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            imputed = self.model(t).cpu().numpy()

        return self._denorm(imputed)
