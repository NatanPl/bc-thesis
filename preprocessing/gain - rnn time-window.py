    ...
    * `shape == (n_indicators,)` for a single sample.
    ...
    def __init__(
        ...
        *,
        ...
        # ── Generator ──────────────────────────────────────────────
        # LSTM layer followed by FC layers
        self.gen_lstm = nn.LSTM(
            input_size=2,  # Each indicator + its mask value
        ...
        self.gen_fc = nn.Sequential(
            nn.Linear(self.lstm_hidden_dim, self.hidden_dim // 2),
            nn.LeakyReLU(),
        ...
        # ── Discriminator ──────────────────────────────────────────
        # LSTM layer followed by FC layers
        self.disc_lstm = nn.LSTM(
            input_size=2,  # Each completed value + hint value
            hidden_size=self.lstm_hidden_dim,
            batch_first=True,
            num_layers=1
        )
    ...
    def _apply_generator(self, x_in, mask):
        """Apply the generator with the LSTM architecture"""
        batch_size = x_in.size(0)
        # Reshape for LSTM: [batch, features] -> [batch, features, 2]
        lstm_input = torch.stack([x_in, mask], dim=2)  # [batch, input_dim, 2]

        # Process with LSTM
        # [batch, input_dim, lstm_hidden_dim]
        lstm_out, _ = self.gen_lstm(lstm_input)

        # Take the last output for each sequence
        lstm_features = lstm_out[:, -1, :]  # [batch, lstm_hidden_dim]

        # Process with FC layers
        return self.gen_fc(lstm_features)

    def _apply_discriminator(self, completed, hint):
        """Apply the discriminator with the LSTM architecture"""
        batch_size = completed.size(0)
        # Reshape for LSTM: [batch, features] -> [batch, features, 2]
        # [batch, input_dim, 2]
        lstm_input = torch.stack([completed, hint], dim=2)

        # Process with LSTM
        # [batch, input_dim, lstm_hidden_dim]
        lstm_out, _ = self.disc_lstm(lstm_input)

        # Take the last output for each sequence
        lstm_features = lstm_out[:, -1, :]  # [batch, lstm_hidden_dim]

        # Process with FC layers
        return self.disc_fc(lstm_features)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:  # (B?, I)
        ...
        gen_out = self._apply_generator(x_in, mask)
        completed = x_in * mask + gen_out * (1 - mask)

        return completed.squeeze(0) if added_batch else completed

    def configure_optimizers(self):
        # Get all discriminator parameters (LSTM + FC)
        disc_params = list(self.disc_lstm.parameters()) + \
            list(self.disc_fc.parameters())

        # Get all generator parameters (LSTM + FC)
        gen_params = list(self.gen_lstm.parameters()) + \
            list(self.gen_fc.parameters())

        opt_d = torch.optim.Adam(disc_params, lr=self.lr * 0.5)
    ...
    def _gen_step(self, flat, orig_observ):
        ...
        logits = self._apply_discriminator(completed, hint)
    ...
    def training_step(self, batch, batch_idx):
        ...
        logits_d = self._apply_discriminator(completed, hint)
    ...
    def transform(self, data: np.ndarray) -> np.ndarray:
        ...
        for c in range(n_countries):
            for start in range(0, n_years - W + 1, step):
                ...
                mapping.append((c, start))
        t = torch.tensor(np.asarray(windows, dtype=torch.float32))
