...
class GAINImputer:
    ...
    def fit(self, data: np.ndarray):
        ...
        reshaped_data = norm.transpose(0, 2, 1).reshape(-1, n_indicators)
    ...
    def transform(self, data: np.ndarray) -> np.ndarray:
        ...
        # Convert to tensor
        t = torch.tensor(reshaped_data, dtype=torch.float32)
        self.model.eval()
        ...
        imputed_reshaped = imputed_flat.reshape(
            n_countries, n_years, n_indicators).transpose(0, 2, 1)

        # Denormalize
        return self._denorm(imputed_reshaped)
